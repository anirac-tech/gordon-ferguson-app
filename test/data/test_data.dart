...
List<Post> get mockPosts => postData.map(Post.fromJson).toList();

rf.HttpResponse<List<Post>> get mockHttpResponse {
  final headers = Headers()
    ..add('X-WP-TotalPages', '${mockPosts.length ~/ pageSize + 1}')
    ..add('X-WP-Total', '${mockPosts.length}');
...
rf.HttpResponse<List<Post>> mockHttpPaginationResponse(int page, int totalLength) {
  assert(totalLength > 0 && page > 0, 'totalLength and page must be greater than 0');
  final pages = totalLength ~/ pageSize + 1;
  final length = min(pageSize, totalLength - (page - 1) * pageSize);
  final headers = Headers()
    ..add(PostResponse.totalPagesHeader, '$pages')
    ..add(PostResponse.totalHeader, '$totalLength');
...
PostResponse get mockPostResponse => PostResponse(
  posts: mockPosts,
  totalPages: mockPosts.length ~/ pageSize + 1,
