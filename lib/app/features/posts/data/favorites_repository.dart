...
class FavoritesRepository {
  const FavoritesRepository({required this.database, required this.log});

  final WpaDatabase database;
  final LogManager log;

  Future<int> addFavorite(Post post) async {
    await log.logEvent(name: 'add_favorite', parameters: {'post': post.id});
    try {
      return database.insertFavoritePost(FavoritePostsCompanion.insert(post: post));
    } catch (e) {
      await log.e(e.toString(), e);
      return -1;
    }
  }

  Future<int> removeFavorite(Post post) async {
    await log.logEvent(name: 'remove_favorite', parameters: {'post': post.id});
    try {
...
Stream<List<Post>> favoriteList(Ref ref) {
  final database = ref.watch(databaseProvider);
