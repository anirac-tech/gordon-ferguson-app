...
class FavoritesView extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: const WpaAppBar(title: Text('Favorites'), actions: [SettingsIconButton()]),
    body: Center(
      child: Consumer(
        builder: (context, ref, child) {
          final posts = ref.watch(favoriteListProvider);
          ref
              .watch(logManagerProvider)
              .d("[Favorites Stream] ${posts.value?.map((e) => '${e.id}')}");
          ...
          return AsyncValueWidget<List<Post>>(
            ...
            data: (data) => data.isNotEmpty
                ...
                : const Text('You have nothing in your favorites.', textAlign: TextAlign.center),
          );
        },
      ),
    ),
